...
- `GET /api/v1/automation/jobs/handoff/` (read-only execution handoff feed for OpenClaw/adapter consumers, throttle scope: `handoff`; filters: `q`, `name`, `status`, `risk_level`, `approval_status`, `assigned_agent_key_id`, `limit`; items include runner assignment/report key fields for executor visibility)
