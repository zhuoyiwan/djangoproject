...
class AgentHMACAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        if not getattr(settings, "AGENT_INGEST_ENABLED", False):
            self._audit_failure(request, "agent_ingest_disabled", "agent:disabled")
            raise AuthenticationFailed("Agent ingest is disabled.")
        ...
        expected_key_id = getattr(settings, "AGENT_INGEST_HMAC_KEY_ID", "")
        secret = getattr(settings, "AGENT_INGEST_HMAC_SECRET", "")
        ...
        tolerance = int(getattr(settings, "AGENT_INGEST_TIMESTAMP_TOLERANCE_SECONDS", 300))
        ...
        prefix = getattr(settings, "AGENT_INGEST_REPLAY_CACHE_PREFIX", "agent_ingest:replay")
