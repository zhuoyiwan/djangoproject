...
class JobApiTests(TestCase):
    ...
    def test_handoff_returns_ready_and_claimed_jobs(self):
        ...
        self.assertEqual(response.data["items"][0]["ready_by_username"], "alice")
        self.assertEqual(response.data["items"][0]["assigned_agent_key_id"], "")
        self.assertEqual(response.data["items"][0]["last_reported_by_agent_key"], "")
        self.assertEqual(response.data["items"][0]["payload"], {"target": "prod"})
    ...
    def test_platform_admin_can_complete_agent_claimed_job(self):
        self.platform_admin.groups.add(Group.objects.create(name=ROLE_PLATFORM_ADMIN))
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
    ...
    def test_platform_admin_can_fail_agent_claimed_job(self):
        self.platform_admin.groups.add(Group.objects.create(name=ROLE_PLATFORM_ADMIN))
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
    ...
    def test_platform_admin_can_cancel_agent_claimed_job(self):
        self.platform_admin.groups.add(Group.objects.create(name=ROLE_PLATFORM_ADMIN))
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
    ...
    def test_requeue_claimed_job_clears_stale_execution_report_fields(self):
        self.user.groups.add(Group.objects.create(name=ROLE_OPS_ADMIN))
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
    ...
    def test_platform_admin_can_requeue_agent_claimed_job(self):
        self.platform_admin.groups.add(Group.objects.create(name=ROLE_PLATFORM_ADMIN))
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
    ...
    @override_settings(AUTOMATION_AGENT_CLAIM_HMAC_KEYS=None)
    def test_agent_claim_accepts_legacy_single_key_settings(self):
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
            status=JobExecutionStatus.READY,
        ...
        payload = {"summary": "claiming with fallback key settings"}
        headers, body = self._agent_claim_signed_headers(job.id, payload)
    ...
    def test_agent_claim_rejects_summary_above_max_length(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["error"]["code"], "validation_error")
        self.assertEqual(response.data["error"]["details"]["summary"], ["Ensure this field has no more than 2000 characters."])
    ...
    def test_agent_report_completed_without_summary_or_metadata_clears_stale_failure_state(self):
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
            status=JobExecutionStatus.CLAIMED,
            ...
            ready_at=timezone.now(),
            execution_summary="stale summary",
            execution_metadata={"error_code": "timeout"},
            failed_at=timezone.now(),
            last_reported_by_agent_key="stale-agent",
        )
    ...
    @override_settings(AUTOMATION_AGENT_REPORT_HMAC_KEYS=None)
    def test_agent_report_accepts_legacy_single_key_settings(self):
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
            status=JobExecutionStatus.CLAIMED,
        ...
        payload = {"outcome": JobExecutionStatus.COMPLETED, "summary": "completed with fallback key settings"}
        headers, body = self._agent_report_signed_headers(job.id, payload)
    ...
    def test_agent_report_rejects_invalid_outcome(self):
        ...
        payload = {"outcome": JobExecutionStatus.CANCELED}
        headers, body = self._agent_report_signed_headers(job.id, payload)
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["error"]["code"], "validation_error")
        self.assertEqual(response.data["error"]["details"]["outcome"], [f'"{JobExecutionStatus.CANCELED}" is not a valid choice.'])

    def test_agent_report_rejects_summary_above_max_length(self):
        job = Job.objects.create(
            name="sync-assets",
            risk_level=JobRiskLevel.LOW,
            status=JobExecutionStatus.CLAIMED,
